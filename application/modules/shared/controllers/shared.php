...
/* 		do {
	...
	function get_items_swg() {
		...
		$result = $this->shared_model->get_easyui_data3($table, $columns, $where, $page, $rows, $sort, $order, $like);
		
		echo json_encode($result);
	}

	function get_opt_phd_routes() {
		if (!$this->ion_auth->logged_in())
			redirect('main', 'refresh');
	...
	function get_opt_phd_routes_central() {
		if (!$this->ion_auth->logged_in())
			redirect('main', 'refresh');
		...
		$company = $this->shared_model->get_company_code( $company_id );
		if ( $company->code == 'FBI' )
			$where	 = array('show_central_fbi'=>1);
		else
		if ( $company->code == 'TGS' )
			$where	 = array('show_central_tgs'=>1);
		else
		if ( $company->code == 'JFI' )
			$where	 = array('show_central_jfi'=>1);
	...
	function get_opt_phd_routes_branch() {
		if (!$this->ion_auth->logged_in())
			redirect('main', 'refresh');
		...
		$company = $this->shared_model->get_company_code( $company_id );
		$branch	= $this->shared_model->get_branch_code( $branch_id );
		if ( $company->code == 'FBI' )
			$where['show_branch_fbi'] = 1;
		else
		if ( $company->code == 'TGS' )
			if ( $branch->code == 'HOF' )
				$where['show_central_tgs'] = 1;
			else
				$where['show_branch_tgs'] = 1;
		else
		if ( $company->code == 'JFI' )
			$where['show_branch_jfi'] = 1;
	...
	function get_opt_phd_status( $department_id=NULL ) {
		if (!$this->ion_auth->logged_in())
			redirect('main', 'refresh');
		...
		$table	 = 'opt_phd_status';
		$columns = NULL;
		$sort	 = !empty($sort)?$sort:'code';
		$order	 = !empty($order)?$order:'asc';
		$where	 = empty($department_id) ? NULL : array('department_id'=>$department_id);
		$like 	 = NULL;
	...
	function get_opt_phd_status_for_central() {
		if (!$this->ion_auth->logged_in())
			redirect('main', 'refresh');
	...
	function get_opt_phd_status_for_central_tgs() {
		if (!$this->ion_auth->logged_in())
			redirect('main', 'refresh');
